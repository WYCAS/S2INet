...
#         super(Generator, self).__init__()
#         # input=100*105
#         # self.h0=torch.randn(2,BATCHSIZE,100).to(device)
#         # self.c0 = torch.randn(2, BATCHSIZE, 100).to(device)
#         # self.lstm1 = torch.nn.LSTM(100, 100, 2, batch_first=True)
#
#         self.fc_layer1=torch.nn.Linear(input,80*80)
...
#         self.h1 = GaborConv2d(in_channels=1, out_channels=128, kernel_size=4,stride=2, padding=1)
#
#         # self.h11 = GaborConv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=1)
#         # self.h8 = GaborConv2d(in_channels=128,out_channels=1, kernel_size=4, stride=2, padding=1)
#         # self.h1=torch.nn.Conv2d(1,128,kernel_size=4,stride=2,padding=1)
#         self.b1=torch.nn.BatchNorm2d(128)
#
...
#         self.b2 = torch.nn.BatchNorm2d(256)
#
#         self.h3 = torch.nn.Conv2d(256, 128, kernel_size=4, stride=2, padding=1)
#         self.b3 = torch.nn.BatchNorm2d(128)
#
#         self.h4 = torch.nn.Conv2d(128, 1, kernel_size=4, stride=2, padding=1)
#         self.b4 = torch.nn.BatchNorm2d(1)
#
#
#
#         self.h5 = torch.nn.ConvTranspose2d(1, 128, kernel_size=4, stride=2, padding=1)
#         self.b5 = torch.nn.BatchNorm2d(128)
#
#         self.h6=torch.nn.ConvTranspose2d(128,256,kernel_size=4,stride=2,padding=1)
#         self.b6=torch.nn.BatchNorm2d(256)
#
#         self.h7 = torch.nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1)
#         self.b7 = torch.nn.BatchNorm2d(128)
...
#         BATCHSIZE=len(c[:, 0])
#         # c=c.permute(0,2,1)
#         # c,(hidden,cell)=self.lstm1(c,(self.h0,self.c0))
#         c = c.reshape(BATCHSIZE, 1, 100*105)
#
#
#         fc1=self.fc_layer1(c)
...
#         # x=torch.cat([z1,fc1],1)
#
#         x=fc1.reshape(BATCHSIZE,1,80,80)
#         x=self.h1(x)
#         x=self.b1(x)
#         x=self.activation(x)
#         x=self.dropout(x)
...
class MLP_GCNN(nn.Module):
    def __init__(self,input):
        ...
        # self.cnn1 = nn.Conv2d(in_channels=1, out_channels=64, kernel_size=(5, 5), padding=2, device=device)
        # self.cnn2 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(5, 5), padding=2, device=device)
        # self.cnn3 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(5, 5), padding=2, device=device)
        # self.cnn4 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=(5, 5), padding=2, device=device)
        # self.cnn5 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(5, 5), padding=2, device=device)
        # self.cnn6 = nn.Conv2d(in_channels=64, out_channels=1, kernel_size=(5, 5), padding=2, device=device)
...
#         out = self.cnn2(out)
        #out=self.relu(out)
        # print('uppool',out.shape)
        # out=out.reshape(1,1,80,80)
